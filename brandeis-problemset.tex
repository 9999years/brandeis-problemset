...
\usepackage{brandeis-problemset}
...
\newcommand{\dep}[1]{\textbf{Warning:} deprecated since #1.\par}
\newcommand{\withopt}[1]{\fbox{With option \option{#1}}}
\makeatletter
\NewDocumentCommand{\withopts}{>{\SplitList{,}} m}
	{\newif\ifopt@first\opt@firsttrue
	\newcommand{\opt@}[1]{\ifopt@first
			\opt@firstfalse
		\else
			,\
		\fi
		\option{##1}}
	\fbox{With any of the
	\ProcessList{#1}{\opt@} options}}
\makeatother
...
\item \ctan{hyperref}, for a nicely-linked table of contents;
	|\href||{url}{label}|.
\item \withopts{listings, scheme, pseudocode, scheme}
\begin{enumerate}
